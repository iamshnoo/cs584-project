...
parser.add_argument(
    ...
    default="nela",
    help="dataset name",
    choices=["liar", "kaggle_fake_news", "tfg", "isot", "ti_cnn", "nela"],
)
    ...
    for epoch in tqdm(range(num_epochs), desc="Epochs", unit="epoch", total=num_epochs):
        ...
        if save_intermediate:
            ...
            filename += datetime.now().strftime(
                f"_%d-%m-%y-%H_%M_{model_name}_epoch{epoch}.pt"
            )
            torch.save(model.state_dict(), f"{models_path}/{filename}")

    filename = DATASET_NAME
    filename += datetime.now().strftime(f"_%d-%m-%y-%H_%M_{model_name}_final.pt")
    torch.save(model.state_dict(), f"{models_path}/{filename}")
    writer.close()
...
def save_test_as_dataframe(all_texts, all_labels, all_preds, split):
    ...
    if split == "test":
        filename += datetime.now().strftime(
            f"_%d-%m-%y-%H_%M_{NAME.replace('/', '-')}_test_results_{sample_size}.csv"
        )
    elif split == "valid":
        filename += datetime.now().strftime(
            f"_%d-%m-%y-%H_%M_{NAME.replace('/', '-')}_valid_results_{sample_size}.csv"
        )
